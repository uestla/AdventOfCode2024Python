...
""")
    ...
    def next_move(map: list[list[str]]) -> list|None:
        ...
        if map[forward_y][forward_x] in ['#', 'O']:
    ...
    for y in range(len(map_original)):
        for x in range(len(map_original[y])):
            ...
            while True:
                ...
                pos_direction_idx = (pos[0] + 1) * pos[1]

                if pos_direction_idx not in pos_directions:
                    pos_directions[pos_direction_idx] = []

                pos_idf = ':'.join(map(str, pos))

                if pos_idf in pos_directions[pos_direction_idx]:
                    obst_loop = True
                    break

                pos_directions[pos_direction_idx].append(pos_idf)
                pos = pos_new
